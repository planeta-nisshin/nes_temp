...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const member = await wpClient.url(`${API_URL}/wp-json/wp/v2/member?offset=${offset}&_embed&per_page=100&before=${date}-31T23:59:59&after=${date}-01T00:00:00`).perPage(PER_PAGE)
  const allpost = await wpClient.url(`${API_URL}/wp-json/wp/v2/member?&per_page=100&before=${date}-31T23:59:59&after=${date}-01T00:00:00`)
  const allcount = Number(allpost.length)
  const memcat = await wpClient.url(`${API_URL}/wp-json/wp/v2/mem_cat?per_page=100`)
  const memdateall = await wpClient.url(`${API_URL}/wp-json/wp/v2/member?per_page=100&before=${date}-31T23:59:59&after=${date}-01T00:00:00`)
...
const MemArchive = ({ members, memcats, allposts, dates,memdateallcounts }: any) => {
  return (
    <div>
      <ul>
        {members.map((mem: any) => (
